...
public class StudyGuideParser
{
    ...
    public static string GetYear(string citation)
    {
        ...
        if (yearString.Length > 1 && !(yearString[1].Contains("изд.") || yearString[1].Contains("диск")))
        {
    ...
    public static ScientificCollection GetScientificCollection(string citation)
    {
        var scientificCollectionString = citation.Replace('–', '-').Split("//");

        if (scientificCollectionString.Length > 1)
        {
            scientificCollectionString = scientificCollectionString[1].Split(". -");
            if (scientificCollectionString[0].Contains("Учебные пособия"))
            {
                return new ScientificCollection() { Title = scientificCollectionString[0].Trim() };
            }
        }
